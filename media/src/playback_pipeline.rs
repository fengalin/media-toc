...
impl PlaybackPipeline {
    pub async fn try_new(
        ...
        let renderer =
            gst::ElementFactory::make(plugin::RENDERER_BIN_NAME, Some("media-toc-renderer-bin"))
                .unwrap();
        renderer
            .set_property(
        ...
        renderer
            .connect(
                plugin::SEEK_DONE_SIGNAL,
                false,
                clone!(@strong int_evt_tx => move |_| {
                    int_evt_tx.unbounded_send(MediaEvent::SeekDone).unwrap();
                    None
                }),
            )
            .unwrap();
    ...
    fn build_audio_pipeline(
        ...
        let renderer_fakesink =
            gst::ElementFactory::make("fakesink", Some("renderer-fakesink")).unwrap();
        renderer_fakesink.set_property("sync", &false).unwrap();
        renderer_fakesink.set_property("async", &false).unwrap();
        ...
        src_pad
            ...
            .unwrap();
        renderer
            .get_static_pad("renderer_src")
            .unwrap()
            .link(&renderer_fakesink.get_static_pad("sink").unwrap())
            .unwrap();
        renderer
...
impl PlaybackPipeline {
    ...
    pub async fn two_steps_seek(
        ...
        start_ts: Timestamp,
        target: Timestamp,
        _flags: gst::SeekFlags,
    ) -> Result<(), SeekError> {
        self.purge_int_evt_()?;

        // FIXME using gst::SeekFlags::ACCURATE otheriwse we don't necessarily get
        // to the target in get stuck waiting for AsyncDone
        ...
        self.pipeline.send_event(
            ...
            .other_fields(&[(plugin::SEEK_TARGET_FIELD, &target.as_u64())])
            .build(),
        );
    ...
    */
    ...
    pub fn seek_range(&self, start: Timestamp, end: Timestamp) {
        ...
        // FIXME
        //self.dbl_audio_buffer.lock().unwrap().ignore_eos();
        ...
        if self.pipeline.set_state(gst::State::Playing).is_err() {
            ...
            // FIXME
            //self.dbl_audio_buffer.lock().unwrap().accept_eos();
        };
    }
