...
impl WaveformImage {
    ...
    fn draw_samples(
        ...
        // FIXME: could use a Peekable here?
        ...
        sample
            ...
            .for_each(|y_value| {
                first_y_values.push(y_value);
                last_y_values.push(y_value);
            });
        ...
        sample_iter.for_each(|sample| {
            ...
            sample
                ...
                .map(|channel_value| self.sample_value_to_y(*channel_value))
                .enumerate()
                .for_each(|(channel, y_value)| {
                    self.set_channel_color(cr, channel);
                    cr.move_to(prev_x, last_y_values[channel]);
                    cr.line_to(x, y_value);
                    cr.stroke();

                    last_y_values[channel] = y_value;
                });
        });
