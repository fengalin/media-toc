    ...
    fn setup(gtk_app: &gtk::Application, main_ctrl_rc: &Rc<RefCell<MainController>>) {
        ...
        audio_ctrl
            ...
            .connect_draw(move |drawing_area, cairo_ctx| {
                match main_ctrl_draw.try_borrow_mut() {
                    Ok(mut main_ctrl) => {
        ...
        // widget size changed
        let main_ctrl_size = Rc::clone(main_ctrl_rc);
        audio_ctrl
            .drawingarea
            .connect_size_allocate(move |_, alloc| match main_ctrl_size.try_borrow_mut() {
                Ok(mut main_ctrl) => {
        ...
        let main_ctrl_motion = Rc::clone(main_ctrl_rc);
        audio_ctrl
            .drawingarea
            .connect_motion_notify_event(move |_, event_motion| {
                let mut main_ctrl = main_ctrl_motion.borrow_mut();
        ...
        // Leave drawing_area
        let main_ctrl_leave = Rc::clone(main_ctrl_rc);
        audio_ctrl
            .drawingarea
            .connect_leave_notify_event(move |_, _event_crossing| {
                let main_ctrl = main_ctrl_leave.borrow();
        ...
        let main_ctrl_pressed = Rc::clone(main_ctrl_rc);
        audio_ctrl
            .drawingarea
        ...
        let main_ctrl_released = Rc::clone(main_ctrl_rc);
        audio_ctrl
            .drawingarea
            .connect_button_release_event(move |_, event_button| {
                if 1 == event_button.get_button() {
                    // left button
                    let mut main_ctrl = main_ctrl_released.borrow_mut();
                    let mut audio_ctrl = &mut main_ctrl.audio_ctrl;
        ...
        let main_ctrl_zoom_in = Rc::clone(main_ctrl_rc);
        zoom_in.connect_activate(move |_, _| {
            main_ctrl_zoom_in.borrow_mut().audio_ctrl.zoom_in();
        });
        ...
        let main_ctrl_zoom_out = Rc::clone(main_ctrl_rc);
        zoom_out.connect_activate(move |_, _| {
            main_ctrl_zoom_out.borrow_mut().audio_ctrl.zoom_out();
        });
        ...
        let main_ctrl_forward = Rc::clone(main_ctrl_rc);
        step_forward.connect_activate(move |_, _| {
            let mut main_ctrl = main_ctrl_forward.borrow_mut();
        ...
        let main_ctrl_back = Rc::clone(main_ctrl_rc);
        step_back.connect_activate(move |_, _| {
            let mut main_ctrl = main_ctrl_back.borrow_mut();
            let seek_pos = {
        ...
        let main_ctrl_async_update = Rc::clone(main_ctrl_rc);
        audio_ctrl.update_conditions_async = Some(Rc::new(move || {
            main_ctrl_async_update
                .borrow_mut()
                .audio_ctrl
                .update_conditions();
        }));
        ...
        // Tick callback
        let main_ctrl_tick = Rc::clone(main_ctrl_rc);
        audio_ctrl.tick_cb = Some(Rc::new(move |_da, _frame_clock| {
            main_ctrl_tick.borrow_mut().audio_ctrl.tick();
        }));
    }
}
