...
impl UIDispatcher for InfoDispatcher {
    fn setup(gtk_app: &gtk::Application, main_ctrl_rc: &Rc<RefCell<MainController>>) {
        ...
        let main_ctrl_show_chap_btn = Rc::clone(main_ctrl_rc);
        info_ctrl
            .show_chapters_btn
            .connect_toggled(move |toggle_button| {
                let main_ctrl = main_ctrl_show_chap_btn.borrow();
        ...
        // Draw thumnail image
        let main_ctrl_draw = Rc::clone(main_ctrl_rc);
        info_ctrl
            .drawingarea
            .connect_draw(move |drawingarea, cairo_ctx| {
                if let Ok(mut main_ctrl) = main_ctrl_draw.try_borrow_mut() {
        ...
        // Scale seek
        let main_ctrl_seek = Rc::clone(main_ctrl_rc);
        info_ctrl
            .timeline_scale
        ...
        // TreeView seek
        let main_ctrl_chap_clicked = Rc::clone(main_ctrl_rc);
        info_ctrl
            .chapter_treeview
            .connect_row_activated(move |_, tree_path, _| {
                let mut main_ctrl = main_ctrl_chap_clicked.borrow_mut();
                let info_ctrl = &mut main_ctrl.info_ctrl;
        ...
        if let Some(ref title_renderer) = info_ctrl.chapter_manager.title_renderer {
            let main_ctrl_chap_edit = Rc::clone(main_ctrl_rc);
            title_renderer.connect_edited(move |_, _tree_path, new_title| {
                let mut main_ctrl = main_ctrl_chap_edit.borrow_mut();
                main_ctrl
                    .info_ctrl
                    .chapter_manager
        ...
        let main_ctrl_add_chap = Rc::clone(main_ctrl_rc);
        add_chapter.connect_activate(move |_, _| {
            let mut main_ctrl = main_ctrl_add_chap.borrow_mut();
        ...
        let main_ctrl_remove_chap = Rc::clone(main_ctrl_rc);
        remove_chapter.connect_activate(move |_, _| {
            main_ctrl_remove_chap
                .borrow_mut()
                .info_ctrl
                .remove_chapter();
        });
        ...
        let main_ctrl_prev_chap = Rc::clone(main_ctrl_rc);
        previous_chapter.connect_activate(move |_, _| {
            let mut main_ctrl = main_ctrl_prev_chap.borrow_mut();
            let position = main_ctrl.get_position();
        ...
        info_ctrl.seek_fn = Some(Rc::new(move |position, seek_flags| {
            main_ctrl_seek.borrow_mut().seek(position, seek_flags);
        }));

        let main_ctrl_show_msg = Rc::clone(main_ctrl_rc);
        info_ctrl.show_msg_fn = Some(Rc::new(move |msg_type, msg| {
            main_ctrl_show_msg.borrow().show_message(msg_type, msg);
        }));
    }
}
